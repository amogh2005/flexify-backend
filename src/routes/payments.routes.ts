...
const formatAmount = (amountInPaise: number): number => {
  return Math.round(amountInPaise / 100);
};

// Utility function to convert rupees to paise
const convertToPaise = (amountInRupees: number): number => {
  return Math.round(amountInRupees * 100);
};

// Create payment order
router.post('/create-order', verifyJwt, async (req: Request, res: Response) => {
  try {
...
// Verify payment
router.post('/verify-payment', verifyJwt, async (req: Request, res: Response) => {
  try {
...
// Get payment details
router.get('/:paymentId', verifyJwt, async (req: Request, res: Response) => {
  try {
...
router.get('/provider/earnings', verifyJwt, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.userId;
    ...
    // Build date filter
    const dateFilter: any = { providerId: provider._id, status: 'completed' };
    ...
    // Get recent payments
    const recentPayments = await PaymentModel.find(dateFilter)
...
// Process refund
router.post('/refund', verifyJwt, async (req: Request, res: Response) => {
  try {
