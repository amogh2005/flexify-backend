...
function calculateTrustScore(providerData) {
    let score = 0;
    // Basic verification
    ...
    if (providerData.category)
        score += 10;
    // Profile enrichment
    if (providerData.portfolio && providerData.portfolio.length > 0)
        score += 15;
    if (providerData.certifications && providerData.certifications.length > 0)
        score += 15;
    if (providerData.languages && providerData.languages.length > 1)
        score += 5;
    // Verification
    ...
    if (providerData.backgroundCheck)
        score += 10;
    if (providerData.skillTestCompleted)
        score += 10;
    // Financial setup
    if (providerData.bankDetails && providerData.bankDetails.accountNumber)
        score += 5;
    ...
    try {
        ...
        if (role === "provider") {
            ...
            try {
                ...
                const provider = new Provider_1.ProviderModel(providerData);
        ...
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ userId: user._id.toString(), role: user.role }, process.env.JWT_SECRET || "fallback_secret", { expiresIn: "7d" });
        // Issue tokens on registration
        const accessToken = jsonwebtoken_1.default.sign({ userId: user._id.toString(), role: user.role }, process.env.JWT_SECRET || "fallback_secret", { expiresIn: "7d" });
        const refreshToken = jsonwebtoken_1.default.sign({ userId: user._id.toString(), role: user.role }, process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET || "fallback_secret", { expiresIn: "30d" });
        res.status(201).json({
...
router.post("/register/provider/step", async (req, res) => {
    try {
        ...
        let provider = await Provider_1.ProviderModel.findOne({ userId });
        if (!provider) {
...
router.post("/refresh", async (req, res) => {
    try {
        ...
        try {
            const decoded = jsonwebtoken_1.default.verify(refreshToken, process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET || "fallback_secret");
            const accessToken = jsonwebtoken_1.default.sign({ userId: decoded.userId, role: decoded.role }, process.env.JWT_SECRET || "fallback_secret", { expiresIn: "7d" });
...
// Get User Profile
router.get("/profile", auth_1.verifyJwt, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: "User not authenticated" });
        }
        const user = await User_1.UserModel.findById(req.user.userId).select("-passwordHash");
        if (!user) {
        ...
        if (user.role === "provider") {
            providerProfile = await Provider_1.ProviderModel.findOne({ userId: user._id });
        }
        res.json({
            user,
            providerProfile,
...
// Update User Profile
router.put("/profile", auth_1.verifyJwt, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: "User not authenticated" });
        }
        ...
        if (user.role === "provider" && Object.keys(otherFields).length > 0) {
            const provider = await Provider_1.ProviderModel.findOne({ userId: user._id });
            if (provider) {
...
// Change Password
router.put("/change-password", auth_1.verifyJwt, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: "User not authenticated" });
        }
